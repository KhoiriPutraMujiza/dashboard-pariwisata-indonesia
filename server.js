...
app.get('/api/hotel_bintang', async (req, res) => {
  try {
    ...
    let query = 'SELECT id, provinsi, tahun, akomodasi, kamar, tempat_tidur FROM jlh_akomodasi_dll_hotel_bintang';
    const params = [];
...
app.get('/api/hotel_non_bintang', async (req, res) => {
  try {
    const { tahun } = req.query;
    let query = 'SELECT id, provinsi, tahun, akomodasi, kamar, tempat_tidur FROM jlh_akomodasi_dll_hotel_non_bintang';
    const params = [];
...
app.get('/api/perjalanan_nusantara', async (req, res) => {
  try {
    ...
    let query = 'SELECT id, provinsi, tahun, jumlah_perjalanan FROM jumlah_perjalanan_pariwisata_nusantara';
    const params = [];
    ...
    console.error('Query error:', err);
    res.status(500).json({ error: 'Gagal mengambil data perjalanan nusantara' });
  }
});
...
app.get('/api/provinsi_geojson', async (req, res) => {
  try {
    const [results] = await pool.query('SELECT province, geojson FROM provinsiind_geojson');
...
app.get('/api/tingkat_hunian', async (req, res) => {
  try {
    ...
    console.error('Query error:', err);
    res.status(500).json({ error: 'Gagal mengambil data tingkat hunian kamar hotel' });
  }
});
